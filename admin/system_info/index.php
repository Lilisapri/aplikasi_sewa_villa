	...
	<div class="card card-outline card-primary">
		...
		<div class="card-body">
			<form action="" id="system-frm" enctype="multipart/form-data">
				...
				<div class="form-group">
					<label for="short_name" class="control-label">Nama Pedek Sistem</label>
					<input type="text" class="form-control form-control-sm" name="short_name" id="short_name" value="<?php echo  $_settings->info('short_name') ?>">
				</div>
				
				<div class="form-group d-flex justify-content-center">
					<img src="" alt="" id="aboutImg" class="img-fluid img-thumbnail" style="display:none;">
				</div>
				<div class="form-group">
					<label for="" class="control-label">S&K</label>
				...
				<div class="form-group">
					<label for="" class="control-label">Cover Sistem</label>
					<div class="custom-file">
...
<script>
	...
	function displayAboutImg(input,_this) {
	    if (input.files && input.files[0]) {
	        var reader = new FileReader();
	        reader.onload = function (e) {
	        	console.log("File loaded: ", input.files[0].name); // Tambahkan log
	        	_this.siblings('.custom-file-label').html(input.files[0].name)
	        	$('#aboutImg').attr('src', e.target.result).show();
	        }
	        reader.readAsDataURL(input.files[0]);
	    }
	}
