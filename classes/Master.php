...
class Master extends DBConnection {
	...
	function delete_room(){ // Menambahkan fungsi untuk menghapus room
		extract($_POST);
		// Pastikan ID tidak kosong
		if(empty($id)){
			$resp['status'] = 'failed';
			$resp['error'] = 'ID tidak boleh kosong.';
			return json_encode($resp);
		}

		// Mengambil upload_path untuk gambar
		$upload_path = $this->conn->query("SELECT upload_path FROM `room_list` WHERE id = '{$id}'")->fetch_assoc()['upload_path'];

		// Menghapus data dari tabel room_list
		...
		if($del){
			...
			if(is_dir(base_app.$upload_path)){
				...
				foreach($file as $img){
					if(in_array($img,array('..','.')))
						continue;
					unlink(base_app.$upload_path.'/'.$img);
				}
				rmdir(base_app.$upload_path);
			}
			$this->settings->set_flashdata('success',"Room successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error; // Menampilkan kesalahan dari query
		}
		return json_encode($resp);
	}
...
switch ($action) {
	...
	break;
	case 'delete_room': // Menambahkan case untuk menghapus room
